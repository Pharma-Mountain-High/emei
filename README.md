...
<img src="images/install1.png" align="center" height="200"/>
...
# R server最简单的用法
result <- emei(
  proj = "QLC5508-301", #项目名
  folder = "~/STA-Server/Projects02/QLC5508/QLC5508-301/SP/dryrun/data/sdtm" #公盘位置
)
```
...
<img src="images/report2.jpg" align="center" height="200"/>

<img src="images/report3.jpg" align="center" height="200"/>

### 📊 解读报告

<img src="images/report1.jpg" align="center" height="500"/>

| 填充颜色 | 含义                                             |
|----------|--------------------------------------------------|
| **白色** | **PASS**：通过                                   |
| **橙色** | **ISSUE**：存在问题，详情请查看对应 **Sheet**    |
| **红色** | **未运行**：缺少所需数据集或变量，程序未成功执行 |
...
result <- emei(
  proj = "QLC5508-201",
  folder = "~/STA-Server/Projects02/QLC5508/QLC5508-201/SP/dryrun/data/sdtm"
)
```
...
### Q: 为什么给定参数路径下不存在某数据集，但是Report中还有该数据集的ISSUE？

A: 检查全局环境是否存在该数据集，建议每次跑ISSUE前清理全局环境。
...
A: 当然可以，前提是要在本地有SDTM数据集，但请尽量在R server平台使用Emei，因为它可以挂载公盘，这样就不用下载数据集到本地了。
