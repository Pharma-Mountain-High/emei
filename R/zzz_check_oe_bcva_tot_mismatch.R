...
check_oe_bcva_tot_mismatch <- function(OE) {
  if (OE %lacks_any% c("USUBJID", "OETESTCD", "OECAT", "OESCAT", "OETSTDTL", "OESTRESN", "OELAT", "VISIT", "OEDTC")) {
    ...
    bcva_t1 <- OE %>%
      filter((OETESTCD == "NUMLCOR" & OESCAT == "TOTAL" & OETSTDTL == "TESTING DISTANCE: 4M" & OECAT == "BEST CORRECTED VISUAL ACUITY") | # Old Standard
    ...
    bcva_t2 <- OE %>%
      filter((OETESTCD == "NUMLCOR" & OESCAT == "TOTAL" & OETSTDTL == "TESTING DISTANCE: 1M" & OECAT == "BEST CORRECTED VISUAL ACUITY") | # Old Standard
    ...
    bcva_t3 <- OE %>%
      filter(OETESTCD == "LCORCON" & OECAT == "BEST CORRECTED VISUAL ACUITY") %>%
      mutate(PARTB_CONST = OESTRESN) %>%
      select(USUBJID, OELAT, VISIT, OEDTC, PARTB_CONST)


    bcva_t4 <- OE %>%
      filter((OETESTCD == "LOGSCORE" & OECAT == "BEST CORRECTED VISUAL ACUITY") | # Old Standard
    ...
    bcva_t <- Reduce(
      function(x, y) merge(x = x, y = y, c("USUBJID", "OELAT", "VISIT", "OEDTC"), all = TRUE),
