#' @title Check AEs with AEDTHDTC value but AESDTH not "是"
#'
#' @description This check looks for AE entries with an AEDTHDTC (death date)
#'   value and AESDTH not equal to "是"
#'
...
check_ae_aedthdtc_aesdth <- function(AE, preproc = identity, ...) {
  ...
  if (AE %lacks_any% c("USUBJID", "AEDTHDTC", "AETERM", "AESDTH", "AEDECOD", "AESTDTC")) {
    ...
    no_aesdth <- !(AE[["AESDTH"]] == "是") | (is_sas_na(AE[["AESDTH"]]))
    ...
    if (nrow(df) == 0) {
      ...
      fail(paste0("AE has ", nrow(df), " record(s) with AESDTH not equal to '是' where AEDTHDTC has a value. "), df)
    }
  }
}
