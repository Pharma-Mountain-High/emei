...
check_ex_exdose_exoccur <- function(EX, drug = NULL) {
  if (EX %lacks_any% c("USUBJID", "EXTRT", "EXSTDTC", "EXDOSE")) {
    ...
    if (!is.null(drug) && !(drug %in% EX[["EXTRT"]])) {
      ...
      # Subsets EX to rows where EXOCCUR == "是" but EXDOSE is missing
      df <- EX %>%
        select("USUBJID", "EXTRT", "VISIT", "EXSTDTC", "EXOCCUR", "EXDOSE") %>%
        filter(EXOCCUR == "是" & is_sas_na(EX[["EXDOSE"]]))
    }
    ...
    # EXOCCUR == "是" and EXDOSE is missing, and whether a drug name is supplied
    if (nrow(df) != 0 && !is.null(drug)) {
      fail(paste0("EX has ", nrow(df), " record(s) with missing EXDOSE when EXOCCUR = '是' (or EXOCCUR does not exist) for ", drug, ". "), df)
    } else if (nrow(df) != 0) {
      fail(paste0("EX has ", nrow(df), " record(s) with missing EXDOSE when EXOCCUR = '是' (or EXOCCUR does not exist). "), df)
    } else {
      pass()
    }
  }
