...
check_tu_rs_new_lesions <- function(RS, TU) {
  ...
  if (RS %lacks_any% "RSEVAL") {
    ...
    myrs <- unique(subset(RS, RS$RSTESTCD == "OVRLRESP" & RS$RSSTRESC %in% c("PD", "PMD") & (toupper(RS$RSEVAL) == "研究者" | is_sas_na(RS$RSEVAL)), c("USUBJID")))
  }
  ...
  if (nrow(mydf) == 0) {
    ...
    fail(
      paste("TU has ", length(unique(mydf$USUBJID)),
        " patient(s) with a new lesion but no Overall Response indicating progression. ",
        sep = ""
      ),
      mydf
    )
