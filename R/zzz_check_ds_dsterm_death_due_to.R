...
check_ds_dsterm_death_due_to <- function(DS) {
  ...
  if (DS %lacks_any% c("USUBJID", "DSTERM", "DSDECOD", "DSSTDTC")) {
    fail(lacks_msg(DS, c("USUBJID", "DSTERM", "DSDECOD", "DSSTDTC")))
  } else {
    # DS records with cause of death
    mydf <- subset(DS,
      # grepl("死亡", toupper(DS$DSDECOD)),
      # (DS$DSTERM=="导致死亡" | DS$DSTERM=="导致死亡 "),
      select = c("USUBJID", "DSTERM", "DSDECOD", "DSSTDTC")
    )
    ...
    if (nrow(mydf) == 0) {
      ...
      fail(paste("DS has ",
        nrow(mydf), " records with missing death reason. ",
        sep = ""
      ), mydf)
    }
  }
