...
check_ae_aesdth_aedthdtc <- function(AE, preproc = identity, ...) {
  ...
  if (AE %lacks_any% c("USUBJID", "AEDTHDTC", "AESDTH", "AETERM", "AEDECOD", "AESTDTC")) {
    ...
    # where AESDTH = "是" and AEDTHDTC does not have a value
    if (nrow(df) == 0) {
      pass()
    } else {
      fail(paste0("AE has ", nrow(df), " record(s) with AESDTH equal to '是' where AEDTHDTC does not have a value. "), df)
    }
  }
}
