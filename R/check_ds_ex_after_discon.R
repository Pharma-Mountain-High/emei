...
check_ds_ex_after_discon <- function(DS, EX) {
  ...
  if (DS %lacks_any% c("USUBJID", "DSSTDTC", "DSCAT", "DSSCAT")) {
    ...
    if ("EXOCCUR" %in% names(EX)) {
      EX <- EX %>% filter(EXOCCUR == "是" & (EXDOSE > 0 | (EXDOSE == 0 & grepl("安慰剂", toupper(EXTRT)))))
    } else {
      EX <- EX %>% filter(EXDOSE > 0 | (EXDOSE == 0 & grepl("安慰剂", toupper(EXTRT))))
    }
    ...
    EX_all <- rbind(
      (EX_start %>% select(USUBJID, DATE = EXSTDTC_imp)),
