#' @title Check for death variable consistency when AEOUT=="死亡"
#'
#' @description This check looks for consistency in AESDTH, AEDTHDTC, and
#' AETOXGR (if applicable) when AEOUT is '死亡'.  Note, this check expects
  ...
  if (AE %lacks_any% c("USUBJID", "AEDECOD", "AESTDTC", "AEDTHDTC", "AEOUT", "AESDTH")) {
    ...
    if (AE %has_any% "AETOXGR") { # if var exists
      if (!all(is_sas_na(AE$AETOXGR))) { # Only run check if var is mapped
        outlist[[1]] <- AE %>%
          filter(AEOUT == "死亡" & (is_sas_na(AEDTHDTC) | AETOXGR != 5 | is_sas_na(AETOXGR) | AESDTH != "是" | is_sas_na(AESDTH)))
      }
    }
    ...
    if (AE %has_any% "AESEV") { # if var exists
      if (!all(is_sas_na(AE$AESEV))) { # Only run check if var is mapped
        outlist[[2]] <- AE %>%
          filter(AEOUT == "死亡" & (is_sas_na(AEDTHDTC) | AESEV != "SEVERE" | AESDTH != "是" | is_sas_na(AESDTH)))
      }
    }
    ...
    if (!(AE %has_any% "AESEV" & AE %has_any% "AETOXGR")) {
      outlist[[3]] <- AE %>%
        filter(AEOUT == "死亡" & (is_sas_na(AEDTHDTC) | AESDTH != "是" | is_sas_na(AESDTH)))
    }
