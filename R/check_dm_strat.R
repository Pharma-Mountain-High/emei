...
check_dm_strat <- function(DM) {
  ...
  if (length(rand_vars_found) == 0) {
    fail("Only applicable for randomized studies. Based on DM.RAANDC1, ……, DM.RAANDRN, DM.RAANDR1, ……, DM.RAANDRN no records were found indicating randomized patients.")
  } else {
    mydf1 <- DM %>%
      select(USUBJID, all_of(rand_vars_found)) %>%
      group_by(USUBJID)
    mydf1 <- mydf1 %>%
      mutate(MISFLAG = if_else(Reduce(`|`, across(all_of(rand_vars_found), is.na)), 1, 0)) %>%
      filter(MISFLAG == 1) %>%
      select(-MISFLAG)
    ...
    if ((nrow(mydf1) == 0)) {
      ...
      fail(
        paste0(length(unique(mydf1$USUBJID)), " patient(s) for randomized study where stratification factors are missing. "),
        mydf1
      )
    }
  }
