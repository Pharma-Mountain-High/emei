...
check_oe_bcva_4m_late_early_tot <- function(OE) {
  ...
  if (OE %lacks_any% c("OESPID")) {
    ...
    BCVA_4m <- BCVA_4m %>%
      filter(
        OECAT == "BEST CORRECTED VISUAL ACUITY" & OETSTDTL == "TESTING DISTANCE: 4M" &
          (!OESTAT %in% c("未查", "ND")) & OESCAT != "LOW LUMINANCE"
      )
    ...
    late4m <- anl1 %>%
      left_join(late_rows, by = c("USUBJID", "VISITNUM", "OEDTC", "OELOC", "OELAT")) %>%
    ...
    anl_sum <- anl1 %>%
      ...
      ungroup()
    incorrect4m <- anl_total %>%
      full_join(anl_sum, by = c("USUBJID", "VISIT", "VISITNUM", "OEDTC", "OELOC", "OELAT")) %>%
