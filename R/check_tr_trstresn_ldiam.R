...
check_tr_trstresn_ldiam <- function(TR, preproc = identity, ...) {
  if ((TR %lacks_any% c("USUBJID", "TRTESTCD", "TRDTC", "VISIT", "TRORRES", "TRSTRESN")) || (TR %lacks_all% c("TRLINKID", "TRLNKID"))) {
    ...
    if (TR %lacks_any% "TREVAL") {
      ...
      df <- subset(TR, TR$TRTESTCD == "LDIAM" & is_sas_na(TR$TRSTRESN) & (toupper(TR$TREVAL) == "研究者" | is_sas_na(TR$TREVAL)), myvars)
    }
    ...
    if (nrow(df) == 0) {
      ...
      # x not done
      x <- nrow(df[df$TRORRES %in% c("未查", "ND"), ])
      if ("TRSTAT" %in% names(df)) {
        x <- x + nrow(df[df$TRSTAT == "未查", ])
      }
      # y not evaluable
      y <- nrow(df[df$TRORRES %in% c("未测量", "NE"), ])
      # z done but missing
      z <- nrow(df) - x - y
