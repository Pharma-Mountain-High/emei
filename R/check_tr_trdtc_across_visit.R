...
check_tr_trdtc_across_visit <- function(TR, preproc = identity, ...) {
  ...
  if ((TR %lacks_any% c("USUBJID", "TRDTC", "VISIT", "TRTESTCD"))) {
    ...
    if (TR %lacks_any% "TREVAL") {
      ...
      trsub <- TR %>%
        filter(TRTESTCD == "LDIAM" & (toupper(TREVAL) == "研究者" | is_sas_na(TREVAL))) %>%
        select(USUBJID, TRDTC, VISIT, TRTESTCD, any_of("RAVE"))
    }
    ...
    if (nrow(mydf) == 0) {
      ...
      fail(
        paste("There are", nrow(mydf), "TR Longest Diameter records where the same date occurs accross multiple visits. "),
        mydf
      )
    }
  }
