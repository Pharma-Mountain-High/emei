...
check_oe_bcva_4m_vs_1m_req <- function(OE) {
  ...
  output_variables <- c(
    "USUBJID", "VISIT", "OEDTC", "OELAT", "BCVA_4M_TOTAL",
    "BCVA_1M_TOTAL", "ISSUE"
  )
  ...
  if (OE %lacks_any% required_variables) {
    ...
    BCVA_4m_1m_total <- BCVA_4m_1m_total %>%
      filter(OECAT == "BEST CORRECTED VISUAL ACUITY" &
        OETSTDTL %in% c("TESTING DISTANCE: 4M", "TESTING DISTANCE: 1M") & OESTAT != "未查" &
    ...
    ## Get BCVA 4m dataset
    BCVA_4m <- BCVA_4m_1m_total %>%
      filter(OETSTDTL == "TESTING DISTANCE: 4M") %>%
      select(-OETSTDTL)
    ...
    ## Get BCVA 1m dataset
    BCVA_1m <- BCVA_4m_1m_total %>%
      filter(OETSTDTL == "TESTING DISTANCE: 1M") %>%
      select(-OETSTDTL)
