  ...
  if (DM %lacks_any% "USUBJID") {
    ...
    if (nrow(dm_no_ae) == 0) {
      ...
      if ("EXOCCUR" %in% names(EX)) {
        EX <- EX %>% filter(EX$EXOCCUR == "是")
      }
      myex <- EX %>%
        filter(!is_sas_na(EXSTDTC) &
          (EXDOSE > 0 | (EXDOSE == 0 & grepl("安慰剂", toupper(EXTRT)))) &
      ...
      myds <- DS %>%
        filter(!is_sas_na(DSSTDTC) & DS$DSDECOD == "死亡" & USUBJID %in% dm_no_ae$USUBJID) %>%
